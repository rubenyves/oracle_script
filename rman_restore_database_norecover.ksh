...
#
# NAME
#   rman_restore_database_norecover.ksh
#
# DESCRIPTION                           
#   Complete restore of the database via RAMN 
#  (database restore without recovery)
# 
#   Restrictions of usage in this version : it doesn't work in the following scenarios 
#        - a tablesapce has been added after the last full backup that will be used for restore
#        - a datafile has been added to a tablespace after the last full backup that will be used for restore
#        - control file is miising
#        - directory tree (sub directories) of the database is not present
#
# REMARKS
#
...
#            TIMES (format jjmmaaaa_hhHmm) --> date and time of the full backup files
#
#    Output : 
#            Log file /oradata/<SID>/adm/dbalog/rman_restore_database_norecover_jjmmyyyy_hhHmm.log
#
# MODIFICATIONS
#    Fabrice CHAILLOU (FT/ROSI/DPS/ITE) - 21/09/2006 - Creation - 1.0.0
#    Adrian Benga (FT/OLNC/IBNF/ITE/ECV) - 24/01/2013 - Evolution 1.1.0
#                                          Prepared for CB standard
#                                          Added start/stop of the logical standby replication
#    Adrian BENGA     (OR/IMT/INBF/ITE/ECV) - 29/07/2014 - 1.2.0 - Adaptation for OM G4R4 architecture
#----------------------------------------------------------------------------------------------------
...
then
	...
	Prompt SQL> set autorecovery off
	set autorecovery off
	Prompt SQL> recover database using backup controlfile until cancel;
	recover database using backup controlfile until cancel
	CANCEL
	! sleep 3
	Prompt SQL>alter database open resetlogs;
	alter database open resetlogs;
	Prompt SQL>ALTER DATABASE START LOGICAL STANDBY APPLY IMMEDIATE;
...
if [ $? -eq 3 ]
   then
     role=`file ${ORACLE_HOME}/dbs/init${ORACLE_SID}.ora | awk -F"_" '{print $2}'`
     if [ "$role" = "STANDBY" ]
        then
...
run {
     restore database from tag '${ORACLE_SID} FULL $times';
 }
exit 0;
EOF
}
...
if [ -z "$times" ]
   then
     echo "Usage : $0 <ORACLE_SID> <TIMES>"
     echo "The parameter TIMES (format jjmmaaaa_hhHmm) is missing. Exit."
     exit 1
fi
...
log=/oradata/${ORACLE_SID}/adm/dbalog/rman_restore_database_norecover_$time.log
history=/oradata/${ORACLE_SID}/adm/dbalog/${ORACLE_SID}_history.log
text="Rman Restore Database Norecover  "
...
banner "$text" Begin
day=`echo $times | cut -d_ -f1`
file=/oradata/${ORACLE_SID}/s01/${ORACLE_SID}_RMAN_DATABASE_FULL_$day
ls $file* 1>/dev/null 2>&1
if [ $? -gt 0 ]
   then
     echo "The full save RMAN ${file}* doesn't exist."|tee -a $log
     banner "$text" End
...
echo "\nStep 1 : STOP APPLICATION LISTENER\n"|tee -a $log
