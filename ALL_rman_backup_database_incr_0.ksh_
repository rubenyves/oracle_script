...
# @(#):Version:1.1
#--------------------------------------------------------------------------------------------------------------
#   Script : ALL_rman_backup_database_incr_0.ksh 
#
...
for i in $(grep -Ev "^#|^ " /etc/oratab); do
   ...
   echo Launch : $dbn, check the logfile /oradata/$dbn/adm/dbalog/rman_backup_database_incr0.log
   echo
   /opt/operating/bin/rman_backup_database_incr.ksh $dbn 0 C 1>/oradata/$dbn/adm/dbalog/rman_backup_database_incr0.log 
