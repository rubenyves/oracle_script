...
# @(#):Version:1.1
#--------------------------------------------------------------------------------------------------------------
#   Script : ALL_rman_delete_backups.ksh 
#
...
for i in $(grep -Ev "^#|^ " /etc/oratab); do
   ...
   echo Launch : $dbn, check the logfile /oradata/$dbn/adm/dbalog/rman_delete_backups.log
   echo
   /opt/operating/bin/rman_delete_backups.ksh $dbn  1> /oradata/$dbn/adm/dbalog/rman_delete_backups.log
done
