...
# @(#):Version:1.1
#--------------------------------------------------------------------------------------------------------------
#   Script : ALL_rman_backup_database_incr_1.ksh
#
...
for i in $(grep -Ev "^#|^ " /etc/oratab); do
   ...
   echo Launch : $dbn, check the logfile /oradata/$dbn/adm/dbalog/rman_backup_database_incr1.log
   echo
   /opt/operating/bin/rman_backup_database_incr.ksh $dbn 1 C 1>/oradata/$dbn/adm/dbalog/rman_backup_database_incr1.log 
