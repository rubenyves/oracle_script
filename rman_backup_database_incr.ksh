...
#
# NOM
#   rman_backup_database_incr.ksh
#
# DESCRIPTION
#   Sauvegarde incrementale RMAN d'une base de donnees (niveau 0 ou 1 / differentielle ou cumulative)
#
# REMARQUES
#
...
#            LEVEL      --> niveau d'incrementale (0, 1)
#            TYPE       --> D(ifferentielle) ou C(umulative)      
#
#    En sortie : 
#            Fichier journal /oradata/<SID>/s01/<SID>_rman_backup_database_Incr[diff|cumu]_lvl[0|1]_jjmmyyyy_hhHmm.log
#
# MODIFICATIONS
#    Fabrice CHAILLOU (FT/DROSI/DPS/ISS) - 07/07/2005 - 1.0.0 - Creation
#    Fabrice CHAILLOU (FT/ROSI/DPS/ITE)  - 11/09/2006 - 1.1.0 - Modification fonction database role
#    Adrian Benga (FT/OLNC/IBNF/ITE/ECV) - 24/01/2013 - Evolution 1.2.0
#                                          Prepared for CB standard
#    Akili ZEGAOUI - 09/04/2021 - 1.3.0 - Adding switch logfile after the backup
#----------------------------------------------------------------------------------------------------
...
#     $1 : SQL statement without the word Selectt
#----------------------------------------------------------------------------------------------------
oracle_var()
{
...
#----------------------------------------------------------------------------------------------------
# Incremental level $level RMAN backup : Datafiles, Controlfile, Archives Logs
#----------------------------------------------------------------------------------------------------
rman_backup()
{
target=/
...
run {
backup incremental level=$level $option database format='${repsvg}/%d_RMAN_DATABASE_INCR${type}_LVL${level}_%D%M%Y_BS%s_BP%p_%t' 
tag='$ORACLE_SID ${type} LVL$level $time';
...
if [ -z "$sid" ]
   then
     echo "Usage: $0 {ORACLE_SID} {0 | 1} {D | C}"
   ...
   then
     if [ "$type" = "D" -o "$type" = "C" ]
        then :
          ...
          echo "Usage: $0 {ORACLE_SID} {0 | 1} {D | C}"
          echo "The parameter TYPE (D=differential or C=cumulative) is missing or incorrect. Exit."
          exit 1
     fi
   else
     echo "Usage: $0 {ORACLE_SID} {0 | 1} {D | C}"
     echo "The parameter LEVEL (0 or 1) is missing or incorrect. Exit."
     exit 1
fi
...
log=/oradata/${ORACLE_SID}/s01/${ORACLE_SID}_rman_backup_database_Incr${type}_lvl${level}_$time.log
history=/oradata/${ORACLE_SID}/adm/dbalog/${ORACLE_SID}_history.log
text="Rman Backup Database Incr $type Level $level"
