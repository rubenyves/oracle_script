...
#
# NOM
#   rman_delete_archivelogs.ksh
#
# DESCRIPTION
#   Suppression des fichiers Archives Logs sauvegardes de plus de n jours
#
# REMARQUES
#
...
#            RETENTION  --> retention en jours
#
#    En sortie : 
#            Fichier journal /oradata/<SID>/adm/dbalog/rman_delete_archvelogs_jjmmyyyy_hhHmm.log
#
# MODIFICATIONS
...
#----------------------------------------------------------------------------------------------------
# Remove archive log files older then n days 
#----------------------------------------------------------------------------------------------------
...
run {
     delete force noprompt archivelog all completed before 'sysdate - $ret' backed up 1 times to device type disk;
    }
EOF
#Syntaxe equivalente : delete force noprompt archivelog until time 'sysdate - $ret' backed up 1 times to device type disk;
}
...
if [ -z "$sid" ]
   then
     echo "Usage: $0 {ORACLE_SID} {RETENTION}"
...
case "$ret" in
      [1-9]*) ;;
        *) echo "Usage: $0 {ORACLE_SID} {RETENTION}"; echo "The parameter RETENTION is missing. Exit."; exit 1 ;;
esac

set_var_ora

time=`date "+%d%m%Y_%HH%M"`
log=/oradata/${ORACLE_SID}/adm/dbalog/rman_delete_archivelogs_$time.log
history=/oradata/${ORACLE_SID}/adm/dbalog/${ORACLE_SID}_history.log
text="Rman Delete Archives Logs          "
